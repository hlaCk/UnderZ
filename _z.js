...
if(typeof Array.prototype.add !== 'function')
{
	Array.prototype.add = function(arr)
	{
		...
		{
			return $.extend(true, this, $.extend(true, $(this).toArray(), arr));
		}
	};
}
...
var
	...
	fns = {
		...
		toggle: {
			class: {
				off: true,
				_call: new Function("if(!!!arguments||!!!arguments.length||fns.toggle.off) { return false; } return $(this).toggleClass(arguments[0]);"),
				call: function(){
					if(this.off)
						return;
	...
	var __zDeferring = {
		...
		callbacks: {
			...
			ready: function(f, promise, unique) { 
				var f=f||false,
					promise=$.type(promise).toLowerCase()!='undefined'?promise:true,
					unique=$.type(unique).toLowerCase()!='undefined'?unique:true;
			...
			array: function (arr,callback,sped){
				if($.type(arr).toLowerCase()=='undefined' && $.type(callback).toLowerCase()=='undefined' && $.type(sped).toLowerCase()=='undefined')
		...
		loader: (function() {
		  ...
		  function _load( tag ) {
			return function( url ) {
				...
				{
					$(loadInBody?"body":"head").append('<script type="text/javascript" class="_z-loader" src="'+url+'"></script>');
...
var hook = hook || function(m){ // hook plugins in _z
	...
	var hookFunc = function(f) { 
		if(typeof(f)==typeof({}))
		{
			...
			{
				// $.extend(true, _z.privates, f);
				_z.extend(true, _z.privates, f);
				return hlack;
			}
			// var $res = $.extend(true, hlack,f);
